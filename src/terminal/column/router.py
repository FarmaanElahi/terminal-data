...
def create(
    ...
    return service.create(session, user.id, column_set_in)
...
def get(
    ...
    cs = service.get(session, user.id, column_set_id)
    if not cs:
...
def update(
    ...
    """Update a column set."""
    cs = service.update(session, user.id, column_set_id, column_set_in)
    if not cs:
        raise HTTPException(status_code=404, detail="Column set not found")
    return cs
...
def delete(
    ...
    """Delete a column set."""
    if not service.delete(session, user.id, column_set_id):
        raise HTTPException(status_code=404, detail="Column set not found")
    return {"ok": True}
