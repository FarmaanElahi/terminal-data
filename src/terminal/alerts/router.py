...
async def list_alert_logs(
    ...
    logs, total = await alerts_service.list_alert_logs(
        session,
        current_user.id,
        alert_id=alert_id,
        symbol=symbol,
...
async def mark_logs_read(
    log_ids: list[str],
    current_user: User = Depends(get_current_user),
    session: AsyncSession = Depends(get_session),
) -> dict[str, int]:
    """Mark alert logs as read."""
    count = await alerts_service.mark_logs_read(session, current_user.id, log_ids)
...
async def clear_alert_logs(
    ...
    count = await alerts_service.clear_alert_logs(session, current_user.id)
...
async def delete_alert_log(
    ...
    success = await alerts_service.delete_alert_log(session, current_user.id, log_id)
    if not success:
        raise HTTPException(
...
async def delete_alerts_by_drawing(
    ...
    count = await alerts_service.delete_alerts_by_drawing(
...
async def create_alert(
    ...
    """Create a new alert."""
    alert = await alerts_service.create_alert(session, current_user.id, body)
...
async def update_alert(
    ...
    alert = await alerts_service.get_alert(session, alert_id, current_user.id)
    if not alert:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Alert not found"
        )
    alert = await alerts_service.update_alert(session, alert, body)
...
async def delete_alert(
    ...
    await alerts_service.delete_alert(session, alert)
...
async def activate_alert(
    ...
    alert = await alerts_service.set_alert_status(session, alert, "active")
...
async def pause_alert(
    ...
    alert = await alerts_service.set_alert_status(session, alert, "paused")
